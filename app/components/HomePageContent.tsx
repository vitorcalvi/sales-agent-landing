    ...
    <section className="border-y border-dark-border py-24 bg-dark-surface-low/30">
      <div className="container mx-auto">
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          {t.businessCore.paragraphs.map((paragraph) => (
            <p
...
function HowItWorksSection({ t }: { t: (typeof CONTENT)['en'] }) {
  return (
    <section id="how-it-works" className="section">
      <div className="container mx-auto">
        <div className="text-center mb-20">
          ...
          <p className="text-gray-400 text-xl max-w-2xl mx-auto">{t.howItWorks.subtitle}</p>
        </div>
        <div className="grid md:grid-cols-3 gap-8">
          {t.howItWorks.steps.map((step) => (
            <div
              key={step.step}
              className="glass-card p-8 relative hover:border-primary/30 transition-all"
            >
              <div className="absolute -top-4 -left-4 w-12 h-12 bg-primary rounded-full flex items-center justify-center text-dark-bg font-heading font-bold text-xl">
                {step.step}
              </div>
              <h3 className="text-xl font-heading mb-4 text-white mt-4">{step.title}</h3>
            ...
            <thead>
              <tr className="border-b border-dark-border">
                <th className="py-4 px-6 font-heading text-primary uppercase text-sm tracking-widest">
                  {t.modes.headers.persona}
                </th>
                <th className="py-4 px-6 font-heading text-primary uppercase text-sm tracking-widest">
                  {t.modes.headers.mode}
                </th>
                <th className="py-4 px-6 font-heading text-primary uppercase text-sm tracking-widest">
                  {t.modes.headers.outcome}
                </th>
              </tr>
            </thead>
            <tbody>
              {t.modes.items.map(
                (mode: { persona: string; mode: string; outcome: string; premium?: boolean }) => (
                  <tr
                    key={mode.mode}
                    className="border-b border-dark-border/50 hover:bg-dark-surface-low/50 transition-colors"
                  >
                    <td className="py-6 px-6 text-gray-400">{mode.persona}</td>
                    ...
                    <td className="py-6 px-6 text-white">{mode.outcome}</td>
                  </tr>
                )
              )}
            </tbody>
...
function TestimonialsSection({ t }: { t: (typeof CONTENT)['en'] }) {
  return (
    <section className="section border-t border-dark-border">
      <div className="container mx-auto">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-heading mb-4 tracking-tight">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {t.testimonials.items.map((testimonial) => (
            <div
              ...
              className="glass-card p-8 hover:border-primary/20 transition-all"
            >
              ...
              <div className="border-t border-dark-border pt-4">
                ...
                <p className="text-primary font-mono text-xs mt-2 uppercase tracking-widest">
                  {testimonial.result}
                </p>
              </div>
            </div>
